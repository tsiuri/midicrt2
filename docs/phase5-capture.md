...
| `alert` | Phase 9 Task 2 (`config.panic_on_crit`): `Engine._maybe_panic`'s own two marks, fired together off the SAME stuck-note CRIT transition — `panic.notes_off` (`{"ch"}`, the external All-Notes-Off CC that went out) and `panic.release` (`{"ch", "notes"}`, an internal synthetic note-off per currently-held note on that channel, routed through `_dispatch_to_state` — the SAME analyzer/page fan-out `Engine._handle` uses — but deliberately NOT through `Engine._handle` itself, so it never becomes a `kind="event"` raw-capture line: it's an honest, provenance-marked INTERNAL action, not a fake wire event). `panic.release` is only stamped when there was actually something to release (empty channel → no mark, matching v1's own `request_release` no-op). Both bypass `ActionRegistry.dispatch` the same way `sysex`/`auto`/`shutdown` do — direct `CaptureSink.record_action` calls, since neither is a client-dispatchable action. |
