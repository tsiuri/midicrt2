...
| **Active-row tint + 1s fade-out** | While a pitch is sounding anywhere in the visible window, its ENTIRE row gets a background tint (`_ROLL_ACTIVE_ROW_BASE_RGB=(0,38,12)`, 15%-brightness green at full intensity); when the note leaves, the tint fades linearly over `_ROLL_ACTIVE_ROW_FADE_S=1.0`s across 64 quantized steps (`_roll_row_fade_lut`) | note-on sets a `fade_until = now+1.0s` per pitch; recomputed every frame from `time.monotonic()` | 64-step linear fade-out, 0%→15%→0% | this IS an animation (continuous per-frame fade) | **MISSING** — no such row-tint/fade mechanism found anywhere in `clients/fb/app.py`'s pianoroll renderer. |
| **Overlap flash** | When ≥2 notes' pixel-space bars overlap on the same pitch row, the overlapping pixel range **cycles through each overlapping note's own color** at 16Hz × a note-count multiplier (0.70× for <2 concurrently active in that exact pixel — effectively unreachable since flash only starts at 2 — 0.90× for 2, 1.30× for 3, 1.70× for 4+), using `time.monotonic()`-driven phase indexing (`int(flash_t*flash_hz) % total_phases`) | wall-clock, independent of transport | cycles between full-brightness note colors, no dimming | animation — draws attention to overlapping/re-triggered notes, doubles as an anti-static-image device in busy passages | **MISSING** entirely in v2 — no per-pixel overlap detection or flash cycling in `render_pianoroll_frame`. |
...
| feature | what it looks like / does | timing-or-sync source | brightness/ramp | burn-in role? | interactive keys | v2 status |
|---|---|---|---|---|---|---|
| Bar-graph spectrum | `"█"` column bars, height ∝ normalized dB level per band (log or linear frequency spacing, configurable), 96 bins default | ~30Hz audio callback → smoothed EMA (`SMOOTHING`) | single-brightness fill, no ramp within a bar | none | 20 interactive keys: `[`/`]` bins, `g`/`h` gain, `s`/`a` smoothing, `f`/`v` floor, `c`/`x` ceiling, `j`/`k` display-scale, `z` auto-adapt toggle, `Z` reset scale, `l` lin/log freq, `m` avg/max aggregation, `n`/`N` low-cut, `p` HPF toggle, `,`/`.` device cycle, `0` default device, `r` refresh devices | PRESENT for the bar rendering itself (v2's `spectrum` page, real hardware capture per `phase3-parity.md` ID 9) — but **all 20 retuning keys unbound client-side** (Phase 4 gap; only `audio_device`/`spectrum_bins` are config-file-adjustable in v2 today). |
| Peak-hold tick | — (v1 has **none**) | — | — | — | — | v2 **addition**, not a v1 feature — `-`/hline peak-hold overlay in both renderers, disclosed in `analyzers/spectrum.py`. |
...
|---|---|---|---|---|---|---|
| Row 1 (`SCREEN_ROWS-1`, true bottom) | `plugins/beatflash.py` | 2-char block at column 0: `"██"` (reverse-video in text mode) on the beat, `"  "` off; flips on every quarter-note boundary (`tick//24 != last_tick//24`), auto-clears after 0.1s | MIDI clock (24 PPQN), gated on `running` | binary on/off, no ramp | animation, small but constant — a literal metronome pulse | **DIFFERENT** — v2's `beatflash_glyph` (`clients/chrome.py`) reshapes this into a **4-level decaying-intensity ramp** (`██`/`▓▓`/`▒▒`/`  ` at intensity thresholds 1.0/0.66/0.33/0.0) instead of v1's hard binary flash — disclosed v2 addition, and "stronger on bar vs beat" (only a bar-flash reaches the top `1.0`/`"██"` tier) is also new (v1 flashes identically every beat). |
...
| feature | what it looks like / does | timing-or-sync source | brightness/ramp | burn-in role | v2 status |
|---|---|---|---|---|---|
| **Page-title scrolling marquee** (row 0) | All loaded pages' titles joined (`"[1:Notes]  [2:Send Notes]  ..."`), scrolled left continuously at `HEADER_SCROLL_SPEED=4.0` chars/sec (config: `core.header_scroll_speed`) whenever the joined string is wider than the screen — doubled-string wraparound (`(page_titles+"    ")*2`) so the scroll loops seamlessly | wall-clock (`time.time()`), independent of MIDI/transport | plain, single brightness | **Yes — a first-class, always-on anti-burn-in device**: constantly-moving text at the very top of the screen, present on EVERY page, all the time, regardless of MIDI activity | **MISSING** — neither `clients/fb/app.py` nor `clients/tui.py` has any scroll/marquee mechanism (confirmed by grep: zero "scroll"/"marquee" hits relevant to a header anywhere in either file). v2's page headers are **static** reverse-video bars showing only the CURRENT page's own title, fixed in place every frame — the literal opposite of v1's design intent here. **Build-priority: high** (see summary). |
...
| Autoconnect-log scrolling message (row 1, right-aligned window) | When `AUTOCONNECT_LOG`'s latest message is too long for its allotted window, it gets its OWN independent scroll offset/timer (same `HEADER_SCROLL_SPEED`), separate from the header's | wall-clock | plain | anti-burn-in (same device as the header, applied to a second text stream) | **MISSING** — no equivalent found in either v2 client; v2 has no autoconnect-log concept at all client-side. |
...
Counts exclude DEAD-IN-V1 rows (nothing there ever ran) and rows marked
"not verifiable from client code" pending an engine-analyzer follow-up
read (img2txtviz's wave/ring/shimmer/trail layer — flagged, not counted
either way).

| status | count | rows |
|---|---|---|
| **PRESENT** | 24 | Help text (as data-equivalent)†, CC Monitor, CC Dashboard bar+age, Event Log, Program Changes, pianoroll TUI velocity ramp, pianoroll projection-mode toggle (action exists, unbound), Audio Spectrum bars, Tuner meter+readout, Chord+Key (all 4 field groups), Voice Monitor counts, Config (narrowed-by-design, itself a disclosed PRESENT), Notes chord/scale-conf/key/harmonic-rhythm/motif lines (5 sub-rows), Notes tension bar, sysex command dispatch, screensaver (improved), pagecycle mechanism (present but wrong semantics — see DIFFERENT), psf font renderer port |
| **DIFFERENT** | 13 | Notes chord/scale reverse-spotlight → lost; Notes inside/outside → reshaped+lost reverse; Help page → live-data replacement; beatflash → ramped instead of binary; timeclock TIMER blink → moot on real CRT either way; pagecycle → wrong semantics (idle-gated vs interval-while-playing), explicitly slated for redo; transport metronome dot → dropped (beatflash-only now); pianoroll channel color → rainbow hue cycle instead of monochrome brightness (**the** monochrome-mandate item); img2txtviz → client is a pure mapper, animation richness unverified pending engine-side read; PixelRenderer per-channel ANSI color → dormant, not the deployment path |
| **MISSING** | 26 | Notes per-channel note-NAME list (not just counts); Notes inline CC badge; pianoroll fb dotted pitch-row grid; pianoroll fb dotted bar-guide verticals; pianoroll fb bar/beat timeline strip; pianoroll fb active-row tint+fade; pianoroll fb overlap-flash; pianoroll fb note-bar outline; pianoroll channel-visibility filter keys; pianoroll pitch-window scroll keys; pianoroll style-toggle key; Send Notes keymap (all bound-nothing); pianoroll_exp session-memory browser (whole subsystem, Phase 5); Stuck Heatmap page; TimeSig Exp page; loopprogress scheduler/sysex diagnostic text; zstucknotes PANIC_ON_CRIT + HOLD_AFTER linger (both now slated to build per decisions doc); **header page-title scrolling marquee** (burn-in-critical); **autoconnect-log independent scroll** (burn-in-critical); Audio Spectrum's 20 retuning keys; pianoroll_exp CC-lane rows; img2txtviz audio-reactivity (explicitly skipped per decision); notes badge / mini-roll-spectrum-piano panel (§ below) |
...
2. **Pianoroll fb "paper" grid** (dotted pitch rows + bar-guide verticals
   ...
   touch the same `_render_pianoroll` function.
...
4. **Pagecycle semantics fix** — already fully decided (2026-08-08 doc:
   ...
   playing). Not a design question, just an implementation debt.
5. **Pianoroll active-row fade + overlap-flash** — smaller in screen
   ...
   static rainbow-color scheme currently papers over differently.
