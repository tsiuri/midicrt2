...
| **Solid bar/beat marker row** (the `"Bars"` timeline strip) | One pixel-row: a solid `GREEN_MID` 1px vertical tick at each bar boundary, `GREEN_DIM` at each beat boundary (not bar) | tick-based | 2-level (mid vs dim green) | contributes to paper-grid feel | **Still MISSING** — explicitly out of Phase 8 Task 3's scope (task-3-brief.md asks for "faint beat/bar gridlines behind the roll," the dotted backdrop above, not a second solid chrome row) and no v2 fb renderer currently reserves space for a fourth strip. **Ownership (review fix)**: assigned to Phase 8 Task 4's animation/burn-in catalog (same §9c section as the two other still-MISSING pianoroll rows below it) rather than left unscheduled. Disclosed in `pages/pianoroll.py`'s own "Paper grid" docstring ("Not ported... out of THIS task's scope") rather than silently dropped. |
...
2. **Pianoroll fb "paper" grid** (dotted pitch rows + bar-guide verticals
   ...
   separate solid timeline strip is the one sub-item still not ported,
   disclosed as out of this task's scope and assigned to Task 4's
   catalog (review fix) rather than silently dropped or left unscheduled.
