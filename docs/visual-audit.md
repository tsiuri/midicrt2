...
| feature | what it looks like / does | timing-or-sync source | brightness/ramp | burn-in role? | interactive keys | v2 status |
|---|---|---|---|---|---|---|
| Bar-graph spectrum | `"█"` column bars, height ∝ normalized dB level per band (log or linear frequency spacing, configurable), 96 bins default | **correction from an earlier draft of this audit** — the audio callback fires once per `BLOCKSIZE=1024`-sample block at `DEFAULT_SR=44100`Hz, i.e. ≈**43Hz** (44100/1024 ≈ 43.07), not ~30Hz as previously stated → smoothed EMA (`SMOOTHING`) | single-brightness fill, no ramp within a bar | none | **correction from an earlier draft of this audit** — recounting `keypress()`'s actual `if`-branches gives **23** distinct interactive keys, not 20: `[`/`]` bins, `g`/`h` gain, `s`/`a` smoothing, `f`/`v` floor, `c`/`x` ceiling, `j`/`k` display-scale, `z` auto-adapt toggle, `Z` reset scale, `l` lin/log freq, `m` avg/max aggregation, `n`/`N` low-cut, `p` HPF toggle, `,`/`.` device cycle, `0` default device, `r` refresh devices | **UPDATE (Phase 8 Task 6, keymap revamp)**: `[`/`]` bins is now **PRESENT** — `spectrum.bins {delta}` (real mutable bin-count state added to `SpectrumAnalyzer.adjust_bins`, bound via `engine/keymap.py::DEFAULT_PAGE_KEYMAPS["spectrum"]`), the one representative key this task picked to prove the args-table keymap mechanism against a real args-requiring action. PRESENT for the bar rendering itself otherwise (v2's `spectrum` page, real hardware capture per `phase3-parity.md` ID 9). The remaining **22** retuning keys (gain/smoothing/floor/ceiling/display-scale/auto-adapt/lin-log/agg-mode/low-cut/HPF-toggle/device-cycle) stay **deferred, no assigned phase yet** — each needs its own new analyzer-side mutable state (and, for lin/log + agg-mode, a `compute_bins`/`_band_edges` API change) beyond what a navigation/keymap task's scope covers; not dropped, just not yet ranked into a future phase the way the notes-badge/mini-roll-spectrum-piano panel (§ below, build-priority #6) was assigned to Phase 9. |
...
| Row 2 (`Y_POS_OFFSET=2`) | `plugins/loopprogress.py` | `[   *    ]` 8-cell bar, position = `(tick % (8 bars in ticks)) / total * 8`, only shown (`*` drawn) while `running`; **plus**, to its LEFT, scheduler-health + recent-SysEx-summary text (`sched:ok` / `sched:overload:x,y,z`, and any recent sysex command echo for 5s) | tick-based position; text driven by scheduler diagnostics + `sysex_status`/`sysex_status_time` | plain, single brightness | animation (moving `*`), continuous 8-bar cycle regardless of MIDI content — good ambient anti-static motion | **PARTIAL** — the 8-cell bar itself is PRESENT (`loopprogress_bar`, `LOOPPROGRESS_BAR_WIDTH=8`, same freeze-when-stopped/hidden-`*` behavior). The scheduler-health/SysEx-diagnostic text to its left is **MISSING** (disclosed, no v2 scheduler-health metric exists; `phase3-parity.md` §2). **PARTIALLY RESOLVED, Phase 9 Task 5**: the SysEx-diagnostic half of this text now has a real v2 data source (`engine/sysex_store.py` + `engine/sysex.py::build_status_text`, fed exclusively by actual midicrt CMD-dispatch outcomes, v1's own `plugins/sysex.py:55-57` semantics) — but it is NOT rendered "to the left of the loopprogress bar" as v1 did; it joins the SHARED second chrome row instead (live alert > poly-limit flash > sysex-status > lingering-cleared > timesig priority chain — re-ranked in the Phase 9 close-out fix wave, controller ruling — a disclosed v2-native row-consolidation, see `docs/phase9-instruments.md`'s "Chrome priority chain" section). The scheduler-health half remains **MISSING** (still no v2 scheduler-health metric). |
...
| Global page-switch keymap | `0`–`9` direct page jump; `!`→11, `@`→12, `#`→13, `$`→14, `%`→15, `^`→16, `&`→17 (shifted-digit mnemonic, i.e. `!` is shift-1 but maps to 11, not 1 — a deliberate non-literal mnemonic scheme); `t`/`T`→10 (Tuner); `C`→trigger MIDI capture-to-file; `q`/`Q`/Esc/Ctrl-C→quit | keypress, global (checked after the current page's own `keypress()` first gets a shot) | — | — | **This is the exact keymap the phase-8 revamp needs to reference/replace.** v2 has its own keymap system already (`engine/keymap.py`, `keymap.toml`, live-reloadable, `describe`-reported) per the Phase 4 work referenced throughout `tui.py`/`fb/app.py` — but the specific v1 shifted-digit scheme above (`!@#$%^&*` → pages 11–17) is the concrete mapping to decide whether to preserve, and is exactly what the decisions doc's "number keys + number+modifier for page jumps; letters for per-page functions" ruling will replace. **RESOLVED, Phase 9 Task 0 (docs/phase4-bindings.md §1c):** `DEFAULT_KEYMAP`'s digit/shifted-digit bindings now reproduce this exact `!`→11…`&`→17 scheme (generalized to a `0`–`9`/`ones-digit` formula spanning v1's full 0–19 ID space), as `page.goto` entries baked from `analyzers/marquee.py::PAGE_IDS` — one disclosed deviation: `t`/`T`→10 is NOT reproduced (letters stay reserved for per-page functions under the revamp's own ruling); page 10 ("tuner") is reachable via shift+0 (`")"`) instead, same formula as every other ID. |
...
| **MISSING** | 18 | Notes per-channel note-NAME list (not just counts); Notes inline CC badge (moot on real CRT even in v1, §0.3); pianoroll fb note-bar outline (static visual-clarity device, not an animation — out of this task's animation/burn-in scope by the audit's own categorization); pianoroll channel-visibility filter keys; pianoroll pitch-window scroll keys; pianoroll style-toggle key; Send Notes keymap (all bound-nothing); pianoroll_exp session-memory browser (whole subsystem, Phase 5); Stuck Heatmap page; TimeSig Exp page; loopprogress scheduler-health diagnostic text (the sysex-diagnostic half of this row is now PRESENT, Phase 9 Task 5 — relocated to the shared alerts/poly-limit/sysex/timesig chrome row rather than literally left-of-bar, see row 2's own entry; scheduler-health itself remains MISSING, still no v2 metric); Audio Spectrum's remaining **22** retuning keys (Phase 8 Task 6 moved `[`/`]` bins to PRESENT above, corrected down from 23; the 22 — gain/smoothing/floor/ceiling/display-scale/auto-adapt/lin-log/agg-mode/low-cut/HPF-toggle/device-cycle — stay **deferred, no assigned phase yet**, not dropped, each needing its own new analyzer-side state); pianoroll_exp CC-lane rows; img2txtviz audio-reactivity (explicitly skipped per decision); notes badge / mini-roll-spectrum-piano panel (§ below — build-priority #6, tentatively flagged for "Phase 9 (instruments & tools)" here, but **NOT actually part of Phase 9's approved 7-task scope** (digit-nav, device-identity, panic/linger/poly-log, tuner, web reconnect, sysex manager, capture editor — see `docs/phase9-instruments.md`) — still open, not dropped, needs its own future-phase assignment) |
...
**Counts corrected in final review** (PRESENT 34 / DIFFERENT 13 / MISSING 19 /
N/A 3, above; Task 6's spectrum-bins move to PRESENT bumps the count from
33→34) — Task 7 was the planned live-real-CRT sign-off pass over
