...
| 10 | Tuner (`tuner.py`) | `pages/tuner.py` (page `tuner`) | **Ported and LIVE-WIRED**, Task 6 (math), Phase 9 Task 3 (audio wiring). Pure post-detection math (`freq_to_note`, `tuning_meter`, smoothing/gating) ported in Task 6; Phase 9 Task 3 wired real pitch detection — its own independent `AudioCapture` (not a shared tap with spectrum's, a disclosed architecture decision, see `pages/tuner.py`'s module docstring) feeds `TunerAnalyzer.on_audio_block()` a dependency-free numpy YIN detector (`analyzers/tuner.py::detect_pitch`). **Correction to this row's own prior claim**: "Task 8 confirmed `aubio` installs cleanly on this exact Pi" did NOT hold up under a direct Phase 9 Task 3 re-test — `pip install aubio` into a throwaway venv on this exact Pi fails to BUILD (a real C compile error, aubio 0.4.9's ufunc glue code vs. this venv's numpy C API, not a missing system package) — no corroborating task-8 brief/report survives to explain the discrepancy; this task's finding supersedes it, disclosed rather than silently overwritten. Now in the default page roster (config.py) — degrades gracefully to a "no audio input" placeholder exactly like spectrum when no input device is present. |
...
| `pagecycle.py` | `behaviors/pagecycle.py` | **Ported, v1 semantics RESTORED (2026-08-09, Phase 8 Task 5)** — superseding the Task-9 re-interpretation this row used to describe. That version turned v1's mechanism into idle-*triggered* (no MIDI activity for `pagecycle_idle_s`) cycling through the **whole roster**, and this row previously disclosed (2026-08-07 fix-wave review) that composition made it **unreachable under Config()'s own shipped defaults** — the screensaver's 60s threshold always fired first and permanently blocked it. The 2026-08-08 decisions doc explicitly ruled this OUT ("get page cycling turned back on; it should do the same pages it used to do") and Phase 8 Task 5 restored v1's actual mechanism verbatim: an unconditional wall-clock `pagecycle_interval` (300s) rotating through a *curated subset* (`pagecycle_pages`, v1's `cycle_pages=[1,6,8,9]` mapped to v2 names — 1→`harmony`, 6→`eventlog`, 8→`pianoroll`, 9→`spectrum`, evidence: each v1 page module's own `PAGE_ID`, cross-checked against this table's own §1 rows), suppressed only by a recent **human page action** (`pagecycle_user_pause`, 3600s — v1's keypress-pause, re-interpreted for v2's multiple actors as origin `{client}` ONLY; a learned MIDI binding, a SysEx command, or another behavior does NOT pause it, since none of those is a human at the console — see `behaviors/pagecycle.py`'s own "Origin ruling" docstring section for the full rationale). **The `sysex` origin was INITIALLY (incorrectly) included in the human-pause set by analogy ("a physical button is still a person") — fix round 1 (2026-08-09) verified this against v1 source and reversed it: `plugins/sysex.py`'s `CMD_SWITCH_PAGE` dispatch calls `midicrt.switch_page()` directly and never touches `notify_keypress()`, which only `midicrt.py`'s `keyboard_listener()` calls on a literal physical keystroke — v1 itself treats a SysEx page switch as remote-control/automation, the same category as a learned binding, not a keypress.** The screensaver-page arbitration guard (`tick()` refuses to act while `current_page` IS the screensaver page) is carried over UNCHANGED from Task 9 — it no longer shares any clock with the screensaver at all (pagecycle's timer is now completely independent of `last_activity_ts`), so this guard is the only thing left preventing pagecycle from un-blanking a just-activated screensaver during a long idle stretch. `config.pagecycle_idle_s` is REMOVED (a disclosed breaking config change, pre-cutover); replaced by `pagecycle_interval`/`pagecycle_pages`/`pagecycle_user_pause`. See the Sign-off section below — the composition item this disclosure originally deferred is now RESOLVED, not merely re-scoped. |
...
| `tuner` | **Ported and LIVE-WIRED** — Task 6/Phase 9 Task 3 (§1, ID 10). |
...
- ~~Tuner audio wiring (page 10)~~ — **RESOLVED, Phase 9 Task 3**: live-wired via a dependency-free numpy YIN detector (aubio confirmed NOT viable on this Pi, see ID-10 row above) on the tuner's own independent `AudioCapture`; joined the default page roster.
...
- ~~**`pagecycle` vs `screensaver` composition under shipped defaults (new, 2026-08-07 fix-wave review)**~~ — **RESOLVED (2026-08-09, Phase 8 Task 5)**, not by picking one of the options this bullet originally listed but by removing the shared-clock composition problem itself: `PageCycleBehavior` no longer measures anything off `last_activity_ts` at all (v1 semantics restored — a pure wall-clock `pagecycle_interval`, paused only by a recent human page action), so there is no longer a race between two idle-timers reading the same activity timestamp. The screensaver-page guard (kept unchanged) still arbitrates the one remaining real conflict — both behaviors wanting to set `current_page` — but pagecycle's own rotation is no longer starved under stock config the way this bullet described. See §2's `pagecycle.py` row above for the full mechanism.
