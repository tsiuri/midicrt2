...
def test_manual_override_re_arms_the_idle_reference_a_full_after_s_required():
    # CORRECTED (2nd review pass): this test used to assert the OPPOSITE --
    # that a manual override "reactivates on the next tick" with no grace
    # period. That turned out to be a bug, not an accepted quirk: it broke
    # behaviors/pagecycle.py's OWN "re-arm after a manual escape" fix,
    # since an instantly-reclaiming screensaver would re-block pagecycle
    # before pagecycle's own freshly re-armed idle_s could ever complete
    # uninterrupted -- see behaviors/screensaver.py's module docstring's "A
    # manual override now buys a FRESH after_s grace period" section. A
    # manual escape now re-arms the idle reference to the escape instant,
    # requiring a FULL FRESH after_s of continued idleness -- NOT an
    # immediate re-flap -- before reactivating.
    b = ScreensaverBehavior(enabled=True, after_s=60.0)
    b.tick(now=60.0, last_activity_ts=0.0, current_page="voices")
    b.tick(now=61.0, last_activity_ts=0.0, current_page="harmony")   # manual override
    assert b.tick(now=120.9, last_activity_ts=0.0, current_page="harmony") is None
    assert b.tick(now=121.0, last_activity_ts=0.0, current_page="harmony") == (
        "page.goto", {"name": "screensaver"})
